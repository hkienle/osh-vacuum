    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="flex w-full flex-col gap-0 overflow-y-auto p-0 sm:max-w-[380px]">
        <SheetHeader className="space-y-1 border-b px-4 py-4 pr-12">
          ...
          <SheetDescription>
            {wifiAvailable
              ? 'Connect to your vacuum via WiFi or Bluetooth.'
              : 'Connect to your vacuum via Bluetooth.'}
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex flex-1 flex-col gap-5 px-4 py-4">
          ...
          <div className="space-y-3">
            ...
            {(!wifiAvailable || transport === 'ble') && !bleSupported && bleUnavailableReason && (
              <InfoBox>{bleUnavailableReason}</InfoBox>
            )}

            {(!wifiAvailable || transport === 'ble') && bleSupported && (
              <InfoBox>
                Bluetooth works in Chrome and Edge on desktop and Android.
                {wifiAvailable ? ' Safari does not support Web Bluetooth — use Wi‑Fi there instead.' : ''}
              </InfoBox>
            )}
            ...
            {wifiAvailable && transport === 'wifi' && !embeddedUi && (
