...
export function gfxPrint(
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    drawChar(ctx, code, cx, cy, textSize, color);
    cx += gfxCharWidth(textSize);
  }
}

export function gfxPrintBold(
  ctx: CanvasRenderingContext2D,
  text: string,
  x: number,
  y: number,
  ...
  gfxPrint(ctx, text, x, y, textSize, color);
  gfxPrint(ctx, text, x + 1, y, textSize, color);
}

export function gfxPrintRight(
  ctx: CanvasRenderingContext2D,
  text: string,
  y: number,
  textSize: number,
  ...
  const w = gfxTextWidth(text, textSize);
  gfxPrint(ctx, text, rightEdge - w, y, textSize, color);
}
...
function drawChar(
  ...
  const offset = idx * GLYPH_W;
  ctx.fillStyle = color;
  for (let col = 0; col < GLYPH_W; col++) {
    let line = FONT5X7[offset + col] ?? 0;
    for (let row = 0; row < GLYPH_H; row++) {
      if (line & 0x1) {
        if (textSize === 1) {
          ctx.fillRect(x + col, y + row, 1, 1);
        } else {
          ctx.fillRect(x + col * textSize, y + row * textSize, textSize, textSize);
        }
      }
      line >>= 1;
    }
