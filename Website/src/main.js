...
const formUrlEl = el('form-url');
const fltMassEl = el('flt-mass');
const fltTimeEl = el('flt-time');
// ── Helpers ─────────────────────────────────────────────────────────────────
function getFormUrl() {
    return formUrlEl.value.trim() || 'https://your-form.example.com/feedback';
}
const FEEDBACK_MAIL = 'osh-vac-feedback@proton.me';
function buildFeedbackMailto(pn, part, s) {
...
function save() { saveToStorage(parts, state); }
function parseMassKg(mass) {
...
async function fetchRepo() {
    ...
    try {
        ...
        setRepo('err', `GitHub: ${err.message}`);
    }
}
...
initWorkshopListeners(wsContainer, {
    onToggle(pn) {
        ...
        const card = wsContainer.querySelector(`[data-pn="${pn}"]`);
...
el('btn-expand').addEventListener('click', () => {
    parts.forEach(p => { if (state[p.pn])
        state[p.pn].expanded = true; });
    renderFilteredWorkshop();
});
el('btn-collapse').addEventListener('click', () => {
    parts.forEach(p => { if (state[p.pn])
        state[p.pn].expanded = false; });
    renderFilteredWorkshop();
});
el('btn-reset').addEventListener('click', () => {
    if (!confirm('Reset check states? File links are kept.'))
        return;
