...
async function renderQRCodes(tbody: HTMLElement, getFormUrl: () => string): Promise<void> {
  const base    = getFormUrl();
  const canvases = Array.from(tbody.querySelectorAll<HTMLCanvasElement>('canvas[data-qr-pn]'));
